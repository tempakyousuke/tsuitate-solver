...
└── solver/
    ...
    └── solver.rs       # TsuitateSolver: 指数的反復深化、転置表、候補手生成・ソート
```
...
├── Controls.svelte     # 解探索制御（深さ設定、解探索、キャンセル、JSON入出力）
...
2. **攻め方ターン (OR ノード)**: 候補手のうち1つでも全分岐で詰みに導ければ成功
3. **玉方ターン (AND ノード)**: 全ての観測分岐で詰みに導ける必要がある
4. **転置表**: 失敗した MetaPosition をハッシュでキャッシュし再探索を回避
5. **候補手ソート**: 打ち駒優先 → 玉に近い手 → 接触王手 → 長距離利き駒の順
6. **枝刈り**: メタポジションサイズ上限(5000)、合駒可能マス数制限、候補手数制限
7. **無駄合い判定**: 取り返して再王手→詰みになる合駒を再帰的に判定して省略
8. **合駒の限定**: 同一マスへの打ち駒合駒を最安値の駒種に絞る（ヒューリスティック）
